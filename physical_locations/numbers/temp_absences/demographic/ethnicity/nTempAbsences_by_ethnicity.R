...
nTempAbsences <- function()
{
  ...
  png("nTemp_Absences_by_ethnicity.png");
  ...
  for (i in 1:n_rows_of_plots)
  {
    ...
    for (j in 1:end_column_index)
    {
     ...
     statement <- paste("SELECT person_id, count(physical_location_id) as nTempAbsences", 
                       " from physical_location_records, people",
                       " where physical_location_records.person_id = people.id",
                       " and physical_location_records.physical_location_type = 'PhysicalLocation::TemporaryAbsence'",
                       " and people.", ethnicity, " = true",
                       " group by person_id", 
                       sep = "");
     res <- dbSendQuery(con, statement);
     data <- fetch(res, n = -1);
     edges <- c(0, 5, 10, 15, 20);
     ...
     customHistogram(histogram = histogram, 
         ...
         xLabel = "#Temp absences", yLabel = "Fraction of children");
    }
  }
  dbDisconnect(con);
  dev.off();
